...
const relayout: RelayoutFn = (id, ratio, wrapper) => {
  ...
  if (width) {
    while (left + 1 < right) {
      middle = ~~((left + right) / 2);
      update(middle);
      if (container.clientHeight === height) right = middle;
      else left = middle;
    }
...
export const Provider = defineComponent({
  setup(_props, { slots }) {
    ...
    return () => [createScriptElement(false), slots.default?.()];
  },
});
  ...
  setup(props, { slots, attrs }) {
    ...
    return () => [
      ...
      withDirectives(
        createScriptElement(
          hasProvider,
          `self.${SYMBOL_KEY}(document.currentScript.dataset.ssrId,${props.ratio})`
        ),
